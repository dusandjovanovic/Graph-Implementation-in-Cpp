...
public:
	...
	long inputValue(linkedNode<type, weight>* targetNode) {
		...
		while (node != nullptr) {
			Edge<type, weight>* edge = node->adj;
			while (edge != nullptr) {
				if (edge->destination == targetNode)
					inputValue++;
				edge = edge->link;
			}
			node = node->next;
	...
	void nodeInput() {
		linkedNode<type, weight>* node = start;
		linkedNode<type, weight>* maxInputNode = start;
		long maxInput = 0;
		...
		while (node != nullptr) {
			long foundValue = inputValue(node);
		...
		cout << endl << "Node of value " << maxInputNode->node << " with input count of " << maxInput << endl;
	}
};